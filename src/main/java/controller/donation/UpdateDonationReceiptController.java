...
public class UpdateDonationReceiptController implements Controller {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		DonationReceiptManager manager = DonationReceiptManager.getInstance();
        ...
        if(check) {    // 전송된 요청 메시지의 타입이 multipart 인지 여부를 체크한다. (multipart/form-data)
            ...
            try {
                ...
                for (int i = 0; i < items.size(); ++i) {
                    ...
                    if (item.isFormField()) {  // item이 일반 데이터인 경우
                    	 ...
                    	 else if(item.getFieldName().equals("receiptId")) {
                    		receiptId = Integer.parseInt(value);
                     		receipt.setReceiptId(receiptId);
                     		fileOrder = manager.getMaxOrder(receiptId)+1;
                         	System.out.println("fileOrder: "+fileOrder);
                     		log.debug("step");
                    	 }
                    	 else if(item.getFieldName().equals("receipt_text"))
            ...
            receipt.setCreateDate(null);
            receipt.setUpdateDate(null);
            
            receipt.setImageList(imageList);
            
            //log.debug("ANimalArticle Controller: {}", receipt);
        }      
    	 
    	 try {
    		 manager.update(receipt);
