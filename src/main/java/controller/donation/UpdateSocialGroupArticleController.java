...
public class UpdateSocialGroupArticleController implements Controller{
	private static final Logger log = LoggerFactory.getLogger(UpdateSocialGroupArticleController.class);
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(check) {    // 전송된 요청 메시지의 타입이 multipart 인지 여부를 체크한다. (multipart/form-data)
            ...
            log.debug("UpdateSocialGroupArticleController: {}", article);
        }      
...
//                 manager.update_image(imageList.get(i));
    		 ...
    		 request.setAttribute("disaster", article);
    		 return "/donationForm/socialGroupUpdateForm.jsp"; 
		}
