...
public class DisasterDAO {
   ...
   public int create(DisasterArticle disaster) throws SQLException {
      try {
         ...
         Object[] param1 = new Object[] {disaster.getTitle(), disaster.getCategory(), disaster.getDeadline(), disaster.getBankName(),
                     disaster.getAccHolder(), disaster.getAccNum(), disaster.getIdCheck(), disaster.getDueDate(), disaster.getUsePlan(), disaster.getOtherText(),
                     disaster.getUpdateDate(), disaster.getReceiptCheck(), disaster.getTotalAmount(), disaster.getUserId()};
...
//         System.out.println("DisasterDao" + disaster);
   ...
   public DisasterArticle findDisasterArticle(int article_id) throws SQLException {
      ...
      try {
         ...
         if(rs.next()) {
            DisasterArticle article = new DisasterArticle(article_id, rs.getString("title"), rs.getString("category"), rs.getString("deadline"), rs.getString("bank_name"), rs.getString("acc_holder"), rs.getString("acc_num"), rs.getString("due_date"), rs.getString("use_plan"), rs.getString("other_text"), rs.getString("create_date"), rs.getString("update_date"), rs.getString("receipt_check"), rs.getString("user_id"), rs.getInt("total_amount"), rs.getString("type"), rs.getString("name"), rs.getString("area"), rs.getInt("damage_amount"), rs.getString("situation"));
   ...
   public int update(DisasterArticle article) throws SQLException {
      try {
           ...
           Object[] param1 = new Object[] {article.getTitle(), article.getDueDate(), article.getUsePlan(),
                  article.getOtherText(), article.getUpdateDate(), article.getReceiptCheck(), article.getArticleId()};
